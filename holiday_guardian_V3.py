...
class StatusPopup:
    def __init__(self, running, next_time, session_start):
        self.win = tk.Toplevel()
        self.win.title("假日守护者 状态")
        ...
        tk.Label(self.win, text="🛡️ 假日守护者 Holiday Guardian",
                 font=("Microsoft YaHei", 11, "bold"), bg=BG, fg="#2c3e50").pack(pady=(16,8))

        if running:
            status_text = "状态：运行中 ✅ Running"
            status_color = "#27ae60"
            ...
            if next_time:
                ...
                mins = total_secs // 60
                secs = total_secs % 60
                remain = f"距下次提醒 Next reminder in: {mins}分{secs}秒 / {mins}m{secs}s"
            else:
                remain = "计时准备中... Preparing..."

            tk.Label(self.win, text=status_text,
                     font=("Microsoft YaHei", 10, "bold"),
                     bg=BG, fg=status_color).pack()
            tk.Label(self.win, text=f"本阶段开始 Session start: {session_start}",
                     font=("Microsoft YaHei", 9), bg=BG, fg="#555").pack(pady=(4,0))
            tk.Label(self.win, text=remain,
                     font=("Microsoft YaHei", 10, "bold"),
                     bg=BG, fg="#e67e22").pack(pady=(8,0))
        else:
            tk.Label(self.win, text="状态：已停止 ⛔ Stopped",
                     font=("Microsoft YaHei", 10, "bold"),
                     bg=BG, fg="#e74c3c").pack()
            tk.Label(self.win, text="右键托盘图标选择「启动」\nRight-click tray icon → Start",
                     font=("Microsoft YaHei", 9), bg=BG, fg="#888").pack(pady=(8,0))

        tk.Button(self.win, text="关闭 Close", command=self.win.destroy,
                  font=("Microsoft YaHei", 9), relief="flat",
                  bg="#ddd", padx=12, pady=4).pack(pady=(14,0))
...
class HolidayGuardian:
    ...
    def __init__(self):
        ...
        self.session_start = ""
        self.timer = None
        self.next_popup_time = None   # datetime 记录下次弹窗时间
    ...
    def _setup_tray(self):
        menu = pystray.Menu(
            pystray.MenuItem("⏱ 查看状态 Status",    self._tray_status, default=True),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem("▶ 启动 Start",          self._tray_start),
        ...
        self.tray = pystray.Icon(
            "hg", make_tray_icon("#e74c3c"), "假日守护者（已停止）", menu
        )
    ...
    def _show_status(self):
        StatusPopup(
            ...
            next_time=self.next_popup_time,
            session_start=self.session_start,
        )

    def _set_tray(self, running):
    ...
    def _schedule(self, minutes):
        if not self.running:
            return
        self.next_popup_time = datetime.datetime.now() + datetime.timedelta(minutes=minutes)
